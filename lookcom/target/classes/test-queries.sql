...
-- Запрос №6. Добавить доступность тарифа "Особый+" по "Брянская область" с абонентской платой 301 р.
INSERT INTO
    region_tariff( tariff_id , region , cost )
VALUES
    ( ( SELECT id FROM tariff WHERE tariff.name = 'Особый+' ) , 'Брянская область' , 301 );
...
SET
    cost = 101
WHERE
    tariff_id = ( SELECT id FROM tariff WHERE tariff.name = 'Особый+' ) AND
...
-- Запрос №10. Удалить все записи адресов в домах которых нет маршрутизаторов
SELECT count(*) FROM address;
DELETE FROM address WHERE address.id NOT IN ( SELECT router.address_id FROM router );
SELECT count(*) FROM address;

-- Запрос №11. Удалить тарифный план "Особый" каскадно ещё и жлшлвлра и доступность по регионам
tariffID = SELECT tariff.id FROM tariff WHERE tariff.name = "Особый";
DELETE FROM tariff WHERE tariff.id = tariffID;
DELETE FROM region_tariff WHERE region_tariff.tariff_id = tariffID;
DELETE FROM agreement WHERE agreement.tariff_id = tariffID;
